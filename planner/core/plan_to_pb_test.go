...
func TestColumnToProto(t *testing.T) {
	...
	pcs := util.ColumnsToProto(0, cols, false)
	for _, v := range pcs {
		require.Equal(t, int32(10), v.GetFlag())
	}
	pcs = util.ColumnsToProto(0, cols, true)
	for _, v := range pcs {
		require.Equal(t, int32(10), v.GetFlag())
	}
