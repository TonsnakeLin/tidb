...
func TestSwapColumn(t *testing.T) {
	...
	require.NoError(t, chk1.SwapColumn(nil, 0, chk2, 0))
	checkRef()
	...
	require.NoError(t, chk2.SwapColumn(nil, 1, chk2, 0))
	checkRef()

	// swap the same column in the same chunk
	require.NoError(t, chk2.SwapColumn(nil, 1, chk2, 1))
	checkRef()

	// swap reference and another column
	require.NoError(t, chk2.SwapColumn(nil, 1, chk2, 2))
	checkRef()

	require.NoError(t, chk2.SwapColumn(nil, 2, chk2, 0))
	checkRef()
}
