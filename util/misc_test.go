...
func TestToPB(t *testing.T) {
	...
	assert.Equal(t, "column_id:1 collation:-45 columnLen:-1 decimal:-1 ", ColumnToProto(0, column).String())
	assert.Equal(t, "column_id:1 collation:-45 columnLen:-1 decimal:-1 ", ColumnsToProto(0, []*model.ColumnInfo{column, column2}, false)[0].String())
}
